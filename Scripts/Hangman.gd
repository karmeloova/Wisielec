...
func _ready():
	...
	SignalManager.extra_heart.connect(_on_extra_heart) #Podłączmy sygnał mówiący o tym, że mamy dodatkowe życie
...
func _on_extra_heart() :
	if(bad_letters>0) : part_tab[bad_letters].visible = false; #"Chowamy" jedną część wisielca - dodatkowe życie
